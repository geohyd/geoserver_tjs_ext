...
/**
 * Created with IntelliJ IDEA. User: capote Date: 10/8/12 Time: 1:09 PM To change this template use
...
public class TJSStreamParserTest {
    ...
    public void parseGetFrameworkDescriptions() throws Exception {
        ...
        StreamingParser parser =
                new StreamingParser(new TJSConfiguration(), is, TJS.FrameworkDescriptions);
        FrameworkDescriptionsType frameworkDescriptions =
                (FrameworkDescriptionsType) parser.parse();
        for (int index = 0; index < frameworkDescriptions.getFramework().size(); index++) {
            FrameworkType framework =
                    (FrameworkType) frameworkDescriptions.getFramework().get(index);
